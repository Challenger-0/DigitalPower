# 1 "BSP/TimerChannel.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 415 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "BSP/TimerChannel.cpp" 2
...
std::uint32_t TimerChannel::getReload() {
    return ((timer.getHandle())->Instance->ARR);
}

void TimerChannel::setCompare(std::uint32_t compare) {
    (((channel) == 0x00000000U) ? ((timer.getHandle())->Instance->CCR1 = (compare)) : ((channel) == 0x00000004U) ? ((timer.getHandle())->Instance->CCR2 = (compare)) : ((channel) == 0x00000008U) ? ((timer.getHandle())->Instance->CCR3 = (compare)) : ((channel) == 0x0000000CU) ? ((timer.getHandle())->Instance->CCR4 = (compare)) : ((channel) == 0x00000010U) ? ((timer.getHandle())->Instance->CCR5 = (compare)) : ((timer.getHandle())->Instance->CCR6 = (compare)));
}

std::uint32_t TimerChannel::getCompare() {
    return (((channel) == 0x00000000U) ? ((timer.getHandle())->Instance->CCR1) : ((channel) == 0x00000004U) ? ((timer.getHandle())->Instance->CCR2) : ((channel) == 0x00000008U) ? ((timer.getHandle())->Instance->CCR3) : ((channel) == 0x0000000CU) ? ((timer.getHandle())->Instance->CCR4) : ((channel) == 0x00000010U) ? ((timer.getHandle())->Instance->CCR5) : ((timer.getHandle())->Instance->CCR6));
}
...
bool TimerChannelDMA::generatePluse(const std::uint16_t *buffer, std::size_t size) {
    ...
    if (result == false)
        return result;
    result = ulTaskGenericNotifyTake( ( ( 0 ) ), ( ( ( BaseType_t ) 0 ) ), ( ( TickType_t ) 0xffffffffUL ) ) == (unsigned long)true;

    return result;
}

void TimerChannelDMA::_txCmptCb() {
    xTaskGenericNotifyFromISR( ( currentTask ), ( ( 0 ) ), ( (unsigned long)true ), ( eNotifyAction::eSetValueWithOverwrite ), 0, ( 0 ) );
}

void TimerChannelDMA::_txFailCb() {
    xTaskGenericNotifyFromISR( ( currentTask ), ( ( 0 ) ), ( (unsigned long)false ), ( eNotifyAction::eSetValueWithOverwrite ), 0, ( 0 ) );
}
