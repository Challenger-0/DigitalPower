# 1 "Onboards/NorFlash.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 415 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Onboards/NorFlash.cpp" 2
# 1 "Onboards/NorFlash.hpp" 1
...
bool NorFlash::eraseSector(std::size_t addr) {
    std::uint8_t cmd[4] = {
        0x20,
        (std::uint8_t)((addr >> 16) & 0xFF),
        (std::uint8_t)((addr >> 8) & 0xFF),
        (std::uint8_t)((addr >> 0) & 0xFF),
    };
    spi.tryLock();
    if (spi.write(cmd, sizeof(cmd)) == false)
        goto FAIL;
